...
export const getRows = async (table: string, query: any = {}): Promise<any[]> => {
	try {
		let rows: any[] = []
		await db
			.select()
			.table(table)
			.where(...formatWhere(query))
			.orderBy(...formatOrderBy(query))
			.limit(query.limit ?? 10000)
			.offset(query.offset ?? 0)
			.then((res: any[]) => (rows = res))
...
const formatWhere = (query: Query): Array<object> | Array<number> => {
	let where: any = {}
	for (const param of Object.keys(query)) if (param !== 'limit' && param !== 'offset' && param !== 'orderby' && param !== 'order') where[param] = query[param]
