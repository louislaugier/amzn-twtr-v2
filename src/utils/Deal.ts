...
export const parseDeals = async (page: Page, dealsGridSelector: string): Promise<Deal[]> => {
	const htmlDealsGrid: string = (await page.$eval(dealsGridSelector, (elem: any) => elem.innerHTML))
		.toString()
		.replace(/&amp;/g, '')
		.replace(/&quot;/g, '')
	...
	for (const deal of htmlDeals) {
		...
		if (!(newDeal.oldPrice && newDeal.newPrice && newDeal.newPrice > newDeal.oldPrice) && !newDeal.url.includes('b?node=')) deals.push(newDeal)
	}
